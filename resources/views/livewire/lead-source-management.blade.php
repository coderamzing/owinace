<div>
    ...
    @if(isset($noTeam) && $noTeam)
        <div class="mb-4 p-4 bg-yellow-100 border border-yellow-400 text-yellow-700 rounded">
            ...
            <p class="text-sm">Please select a team from the team switcher in the top bar to manage lead sources.</p>
        </div>
    @endif

    <div class="flex justify-between items-center mb-4">
        <h3 class="text-lg font-semibold">Lead Sources</h3>
        @if(!isset($noTeam) || !$noTeam)
            <button
                    ...
                    @foreach($sources as $source)
                        <tr>
                            ...
                            <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                                <div class="max-w-xs truncate" title="{{ $source->description }}">
                                    {{ $source->description ?? '-' }}
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    <div class="w-6 h-6 rounded" style="background-color: {{ $source->color }}"></div>
                                    <span class="text-sm text-gray-500 dark:text-gray-400">{{ $source->color }}</span>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 text-xs font-medium rounded {{ $source->is_active ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-200' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end gap-2">
                                    <button
                                        wire:click="openModal({{ $source->id }})"
                                    ...
                                    <button
                                        ...
                                        wire:confirm="Are you sure you want to delete this lead source?"
    ...
    @else
        <div class="text-center py-8 text-gray-500 dark:text-gray-400">
            <p>No lead sources found. Create your first one!</p>
        </div>
    @endif
                            ...
                            <div class="sm:flex sm:items-start">
                                <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
                                    <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-100 mb-4" id="modal-title">
                                        {{ $editingId ? 'Edit' : 'Create' }} Lead Source
                                    </h3>
